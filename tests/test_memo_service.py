...
def test_arguments_passed_through_pipeline(
    ...
    mock_validate.return_value = retrieved_df
    ...
    mock_validate.assert_called_once_with(retrieved_df, high_scores)
...
def test_empty_retrieval_results_produce_no_evidence_memo(
    ...
    mock_validate.return_value = empty_df
    ...
    assert "reason" in kwargs["scoring_info"]
...
def test_validation_filters_all_tickets_produces_no_evidence(
    ...
    assert kwargs["scoring_info"]["score"] == 0
    assert kwargs["scoring_info"]["confidence"] == "Low"
    assert "No relevant customer evidence" in kwargs["scoring_info"]["reason"]
